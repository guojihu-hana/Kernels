...
#define TILE_SIZE 16
template <typename T>
__global__ void gemm_block_naive(
    int M, int N, int K,
    T alpha,
    ...
    T* __restrict__ C)
{
    ...
    for (int k = 0; k < (K + TILE_SIZE - 1) / TILE_SIZE; ++k) {
        shared_A[threadIdx.y * TILE_SIZE + threadIdx.x] = A[row * K + k * TILE_SIZE + threadIdx.y * K + threadIdx.x];
        shared_B[threadIdx.y * TILE_SIZE + threadIdx.x] = B[k * N * TILE_SIZE + col + threadIdx.y * N + threadIdx.x];

        __syncthreads();

        for (int i = 0; i < TILE_SIZE; ++i) {
            acc += shared_A[threadIdx.y * TILE_SIZE + i] * shared_B[i * TILE_SIZE + threadIdx.x];
        }
        __syncthreads();
    }
    int idx = (row + threadIdx.y) * N + col + threadIdx.x;
    C[idx] = alpha * acc + beta * C[idx];
}

// Clear the g_row and g_col
#define TILE_SIZE 16
template <typename T>
__global__ void gemm_block_float4_reg(
    int M, int N, int K,
    T alpha,
    const T* __restrict__ A,
    const T* __restrict__ B,
    ...
    T* __restrict__ C)
{
    ...
    const int g_row = row + threadIdx.y;
    const int g_col = col + threadIdx.x;

    T acc = static_cast<T>(0);
    for (int kt = 0; kt < (K + TILE_SIZE - 1) / TILE_SIZE; ++kt) {
        const int a_row = g_row;
        const int a_col = kt * TILE_SIZE + threadIdx.x;
        const int b_row = kt * TILE_SIZE + threadIdx.y;
        const int b_col = g_col;

        T va = static_cast<T>(0);
        if (a_row < M && a_col < K)
            va = A[a_row * K + a_col];
        T vb = static_cast<T>(0);
        if (b_row < K && b_col < N)
            vb = B[b_row * N + b_col];

        shared_A[threadIdx.y * TILE_SIZE + threadIdx.x] = va;
        shared_B[threadIdx.y * TILE_SIZE + threadIdx.x] = vb;
...
int main()
{
    ...
    for (int r = 0; r < kWarmupRuns; ++r) {
        gemm_block_float4_reg<T><<<grid, block>>>(M, N, K, alpha, dA, dB, beta, dC);
