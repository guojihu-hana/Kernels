...
__global__ void gemm_block_256_4(
     int M,
     int N,
     int K,
     float alpha,
 ...
 #pragma unroll
     ...
     const int load_a_smem_m = tid;
     const int load_a_smem_k = 0;
     const int load_b_smem_k = tid >> 6;
     const int load_b_smem_n = (tid & 63) << 2;
 ...
 int main()
 {
    ...
    constexpr int BM256 = 256;
     constexpr int BN256 = 256;
     dim3 block256(16, 16);
     dim3 grid256((N + BN256 - 1) / BN256, (M + BM256 - 1) / BM256);
 
     cudaEvent_t t2, t3;
     check_cuda(cudaEventCreate(&t2), "event2");
     check_cuda(cudaEventCreate(&t3), "event3");
 
     for (int r = 0; r < kWarmupRuns; ++r) {
         gemm_block_256_4<<<grid256, block256>>>(M, N, K, alpha, dA, dB, beta, dC);
         check_cuda(cudaGetLastError(), "warmup kernel");
     }
     check_cuda(cudaDeviceSynchronize(), "warmup sync");
 
     float total_ms256 = 0.f;
     for (int r = 0; r < kTimedRuns; ++r) {
         check_cuda(cudaEventRecord(t2), "record t2");
         gemm_block_256_4<<<grid256, block256>>>(M, N, K, alpha, dA, dB, beta, dC);
         check_cuda(cudaGetLastError(), "kernel launch");
         check_cuda(cudaEventRecord(t3), "record t3");
         check_cuda(cudaEventSynchronize(t3), "sync t3");
         float ms_iter = 0.f;
         check_cuda(cudaEventElapsedTime(&ms_iter, t2, t3), "elapsed");
         total_ms256 += ms_iter;
     }
     const float ms256 = total_ms256 / static_cast<float>(kTimedRuns);
 
     check_cuda(cudaMemcpy(hC_gpu, dC, szC, cudaMemcpyDeviceToHost), "D2H C");
 
     double flops256 = 2.0 * static_cast<double>(M) * N * K;
     double gflops_naive256 = (flops256 / 1e9) / (static_cast<double>(ms256) / 1000.0);
 
     std::printf("GEMM %dx%dx%d  row-major  gemm_block_256_4 (BM=BN=256, BK=4, TM=TN=16)\n", M, N, K);
     std::printf(
    ...
    cublasHandle_t blas_handle{};
     ...
     double gflops_blas = (flops256 / 1e9) / (static_cast<double>(ms_blas) / 1000.0);
